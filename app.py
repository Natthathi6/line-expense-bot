...
def webhook():
    ...
    for event in events:
        ...
        if text.lower() == "export":
            ...
            reply_text(reply_token, "📦 ดาวน์โหลดวัตถุดิบ: https://your-bot-url.onrender.com/download")
            return "ok", 200
        ...
        for line in lines:
            line = line.strip()
            # รองรับชื่อวัตถุดิบหลายคำ + จำนวน + หน่วย
            match = re.match(r"^(.+?)\s+(\d+(?:\.\d+)?)\s+(\S+)$", line)
            if match:
                item, qty, unit = match.groups()
                records.append((item.strip(), f"{qty} {unit}", date_str, datetime.now().isoformat()))

        if not records:
            reply_text(reply_token, "❌ รูปแบบไม่ถูกต้อง\nควรพิมพ์แบบ: หมูบด 2 ขีด หรือ\n26 Jul 2025\nไข่ไก่ 30 ฟอง")
            return "ok", 200
